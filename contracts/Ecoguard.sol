...
contract Ecoguard is ReentrancyGuard, Pausable {
    ...
    function purchaseNFT(uint256 _listingId) external payable nonReentrant {
        ...
        require(nft.auction == false, "ONLY BID");
    ...
    function setPrice(uint256 _listingId, uint256 newPrice) external {
        ...
        require(msg.sender == nft.seller, "Not authorized");
    ...
    function placeBid(uint256 _listingId, uint256 amount) external payable nonReentrant {
        ...
        require(nft.auction == true, "purchase Only");
    ...
    function finalizeAuction(uint256 _listingId) external nonReentrant {
        NFT storage nft = nfts[_listingId];
        require(nft.available == false || block.timestamp >= nft.listingEndTime, "Auction Active");

        require(msg.sender == nft.highestBidder || msg.sender == nft.seller, "Not authorized");
    ...
    function withdrawNFT(uint256 _listingId) external {
        ...
        IERC721(nft.assetAddress).transferFrom(address(this), nft.seller, nft.tokenId);
    ...
    function setConservationProject(uint256 _listingId, address newConservationProject) external{
        require(msg.sender == nfts[_listingId].seller, "Not authorized");
    ...
    function withdrawPlatformFee() external onlyOwner{
        ...
        IERC20(cusdAddress).transfer(ecoguardWallet, balance);
